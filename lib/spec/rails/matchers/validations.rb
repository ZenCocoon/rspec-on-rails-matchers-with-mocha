  ...
  module Rails
    module Matchers
      def validate_presence_of(attribute)
        return simple_matcher("validates the presence of #{attribute}") do |model|
      ...
      def validate_length_of(attribute, options)
        ...
        return simple_matcher("validates the length of #{attribute} within #{min || 0} and #{max || 'Infinity'}") do |model|
          invalid = false
          if !min.nil? && min >= 1
      ...
      def validate_uniqueness_of(attribute)
        return simple_matcher("validates the uniqueness of #{attribute}") do |model|
      ...
      def validate_confirmation_of(attribute)
        return simple_matcher("validates the confirmation of #{attribute}") do |model|
