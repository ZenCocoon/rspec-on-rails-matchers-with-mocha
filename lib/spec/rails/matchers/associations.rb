  ...
  module Rails
    module Matchers
      def belong_to(association)
        return simple_matcher("belongs to #{association}") do |model|
      ...
      def have_many(association)
        return simple_matcher("have many #{association}") do |model|
          model = model.class if model.is_a? ActiveRecord::Base
      ...
      def have_one(association)
        return simple_matcher("have one #{association}") do |model|
          model = model.class if model.is_a? ActiveRecord::Base
      ...
      def have_and_belong_to_many(association)
        return simple_matcher("have and belong to many #{association}") do |model|
          model = model.class if model.is_a? ActiveRecord::Base
